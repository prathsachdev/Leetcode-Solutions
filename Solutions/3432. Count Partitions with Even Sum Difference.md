...
We use two variables $prefixSum$ and $suffixSum$ to represent the sum of the left subarray and the right subarray, respectively. Initially, $prefixSum = 0$ and $suffixSum = \sum_{i=0}^{n-1} \textit{nums}[i]$.

Next, we traverse the first $n - 1$ elements. Each time, we add the current element to the left subarray and subtract it from the right subarray. Then, we check if $prefixSum - suffixSum$ is even. If it is, we increment the answer by one.
