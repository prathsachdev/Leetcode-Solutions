...
We directly iterate through the array $\textit{nums}$. For each element $num$, if $nums \bmod 3 \neq 0$, there are two cases:

-   If $nums \bmod 3 = 1$, we can decrease $nums$ by $1$ to make it $nums - 1$, which is divisible by $3$.
-   If $nums \bmod 3 = 2$, we can increase $nums$ by $1$ to make it $nums + 1$, which is divisible by $3$.
