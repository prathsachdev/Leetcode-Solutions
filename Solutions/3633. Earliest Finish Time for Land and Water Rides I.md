...
### **Approach 1: Enumeration + Greedy**

We can consider two orders of rides: first land rides then water rides, or first water rides then land rides.

For each order, we first calculate the earliest end time $\textit{min1}$ of the first type of ride, then enumerate the second type of ride and calculate the earliest end time of the second type of ride as $\max(\textit{min1}, \textit{startTime}) + \textit{duration}$, where $\textit{startTime}$ is the start time of the second type of ride. We take the minimum value among all possible earliest end times as the answer.
...
```java
class Solution {
    private int calc(int[] startTime1, int[] duration1, int[] startTime2, int[] duration2) {
        int n1 = startTime1.length;
        int min1 = Integer.MAX_VALUE;
        
        // Calculate min end time of ride1
        for (int i = 0; i < n1; ++i) {
            min1 = Math.min(min1, startTime1[i] + duration1[i]);
        }
        
        int n2 = startTime2.length;
        int min2 = Integer.MAX_VALUE;
        
        // Calculate min end time of ride2
        for (int i = 0; i < n2; ++i) {
            min2 = Math.min(min2, Math.max(min1, startTime2[i]) + duration2[i]);
        }
    ...
    public int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        // Complete land rides first, followed by water rides
        int planA = calc(landStartTime, landDuration, waterStartTime, waterDuration);
        
        // Complete water rides first, followed by land rides
        int planB = calc(waterStartTime, waterDuration, landStartTime, landDuration);
