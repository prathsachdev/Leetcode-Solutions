...
We iterate through each word $word$ in $\textit{words}$, calculate its weight $sum$, which is the sum of the weights of all characters in the word. Then we calculate $sum$ modulo 26, map the result to a lowercase English letter, and finally concatenate all the mapped characters and return.
...
<p><strong>Space Complexity:</strong> $O(W)$</p>
