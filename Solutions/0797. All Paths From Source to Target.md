...
```java
class Solution {
    public void dfsRecursive(int vertex, int end, int[][] graph, List<Integer> dfs, List<List<Integer>> ans) {
        if (vertex == end) {
        ...
        for (int neighbor : graph[vertex]) {
            dfs.add(neighbor);
            dfsRecursive(neighbor, end, graph, dfs, ans);
    ...
    public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
        ...
        dfs.add(start);
        dfsRecursive(start, end, graph, dfs, ans);
