...
The time complexity is $O(|buildings|)$. The space complexity is $O(n)$.

<p><strong>Time Complexity:</strong> $O(|buildings|)$</p>
...
class Solution {
    public int countCoveredBuildings(int n, int[][] buildings) {
        ...
        for (int[] building : buildings) {
            ...
            if (northernmost[x] < y && y < southernmost[x] && westernmost[y] < x && x < easternmost[y]) {
                ++ans;
            }
        }
